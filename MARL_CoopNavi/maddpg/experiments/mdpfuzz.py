    ...
    with U.single_threaded_session():
        ...
        while current_fuzz_time - start_fuzz_time < 3600 * arglist.hour:
            ...
            if terminal and collisions > 5 and not done:
                ...
                init_state = np.array(init_state).flatten().tolist()
                print('time: ', regular_time, '\tfound: ', len(fuzzer.result), '\tcorpus:', len(fuzzer.corpus), 'test_case:\t', init_state)
                fuzz_failure_list.append([regular_time, len(fuzz_failure_list), init_state])
