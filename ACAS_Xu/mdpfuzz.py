...
def sample_seeds(seeds_num=1000):
    ...
    while len(fuzzer.corpus) < seeds_num:
        ...
        if (air1.Vint <= 1200 and air1.Vint >= 0) and (min_dis1 >= dis_threshold or min_dis2 >= dis_threshold):
            ...
            print(acas_speed, x2, y2, auto_theta)
...
def fuzz_testing(fuzzer, args):
    ...
    while len(fuzzer.corpus) > 0:
        ...
        if collide_flag:
            ...
            print('time: ', regular_time, '\tfound: ', len(fuzzer.result), '\tcorpus:', len(fuzzer.corpus), 'test_case:\t', test_case)
            fuzz_failure_list.append([regular_time, len(fuzzer.result), test_case])
