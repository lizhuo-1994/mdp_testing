...
def online_em(K, dataX, S, weights, means, covariances, iteration_num):
    ...
    for i in range(K):
        ...
        # new_S[i][2] = gamma * w[i] * np.matmul(dataX.T, dataX) + (1 - gamma) * (S[i][2] + np.matmul(means[i].T, means[i])) - np.matmul((new_S[i][1] / new_S[i][0]).T, (new_S[i][1] / new_S[i][0]))
        # new_S[i][2] = S[i][2] * (iteration_num / (iteration_num + 1)) + (iteration_num / (iteration_num + 1)**2) * np.matmul((dataX - new_S[i][1]).T, (dataX - new_S[i][1]))
...
if __name__ == '__main__':
    ...
    data = np.array(data)
    # data = np.random.random((500, dimension))
    print(data.shape)
    init_covariances = [np.cov(data.T)] * K
    # print(np.linalg.inv(init_covariances[0]).shape)
    print('start EM')
    # results = EM(data[:500], init_means, init_covariances, init_weights)
    ...
    # means = results['means']
    # covariances = results['covs']
    S = []
    # weights = []
    # covariances = init_covariances
    # means = init_means
    # tempX2 = np.zeros((dimension, dimension))
    # for i in range(500):
    #     tempX2 += np.matmul(data[i:i+1].T, data[i:i+1])
    # tempX2 /= 500
    # for i in range(K):
    #     S[i].append(weights[i])
    #     S[i].append(means[i])
    #     S[i].append(weights[i] * temp)
