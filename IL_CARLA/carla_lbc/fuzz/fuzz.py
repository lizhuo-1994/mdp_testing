...
# TODO: 1. add gmm to fuzzing process
#       2. mutate the whole space instead of the fixed envs (how to make sure the found bugs are unique?)
#       3. find more (all) MDP applications, run fuzzing, (running time)

class fuzzing:
    def __init__(self):
        # HACK: add other cars' position here
        self.corpus = []
        self.rewards = []
        self.result = []
        self.entropy = []
    ...
    def further_mutation(self, current_pose, rewards, entropy, cvg, original, further_envsetting):
        choose_index = self.current_index
        pose = current_pose[0]
    ...
    def mutation(self, pose):
        # HACK: mutate here (cross over...)
        # print('mutated!')
        newpose = carla.Transform(carla.Location(x=pose.location.x, y=pose.location.y, z=pose.location.z), carla.Rotation(pitch=pose.rotation.pitch, yaw=pose.rotation.yaw, roll=pose.rotation.roll))
        newpose.location.x = pose.location.x + np.random.uniform(-0.05, 0.05)
        newpose.location.y = pose.location.y + np.random.uniform(-0.05, 0.05)
        newpose.rotation.yaw = pose.rotation.yaw + np.random.uniform(-1.5, 1.5)
        self.current_pose = newpose
        # pose.rotation.yaw = pose.rotation.yaw - 50
        return newpose
...
if __name__ == '__main__':
    ...
    corpus_size = 100
    for i in range(50):
        cur_seq = np.random.rand(seq_dim)
    ...
    for i in range(corpus_size):
        ...
        for j in range(seq_len):
            cur_seq.append(np.random.rand(seq_dim))
        corpus.append(np.array(cur_seq))
    print(len(corpus), corpus[0].shape)
    
    # states_seq = []
    # for i in range(seq_len):
    #     states_seq.append(np.random.rand(seq_dim))
    # for i in range(10):
    #     if i < 5:
    #         states_seq[0][0] = np.random.uniform(0, 0.1)
    #         states_seq[0][1] = np.random.uniform(0, 0.1)
    #         states_seq[0][2] = np.random.uniform(0, 0.1)
