...
def run(model_path, port, suite, big_cam, seed, autopilot, resume, max_run=10, show=False, replay=False, em_guide=True, args = None):
    ...
    for suite_name in get_suites(suite):
        ...
        with make_suite(suite_name, port=port, big_cam=big_cam) as env:
            ...
            run_benchmark(agent_maker, env, benchmark_dir, seed, resume, max_run=max_run, show=show, replay=replay, em_guide=em_guide, args = args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--emguide', action='store_true', default=False)

    parser.add_argument('--max-run', type=int, default=1)

    ######################## parameters for generative testing ############################################
    parser.add_argument("--method", help="select the guidance for testing", default="mdpfuzz", type=str, required=False)
    ...
    if args.replay:
        ...
        run(Path(args.model_path), args.port, args.suite, args.big_cam, args.seed, args.autopilot, args.resume, max_run=args.max_run, show=args.show, replay=args.replay, em_guide=args.emguide, args = args)