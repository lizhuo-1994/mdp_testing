...
def run_single(env, weather, start, target, agent_maker, seed, show=False, replay=False, em_guide=True, args = None):
    ...
    #test_cases = load_test_cases('carla_lbc/results/fuzz_failure_count.json')
    test_cases = load_test_cases('carla_lbc/results/generative+novelty_diffusion_failure_count.json')

    for i in range(8, len(test_cases)):
        try:
