...
class CarlaWrapper(object):
    ...
    def spawn_vehicles(self):
        if self.replay:
            ...
            for i in range(self.n_vehicles):
                ...
                choose_point = spawn_points[0]
                choose_point.location.x = self.test_settings.vehicles[i][0]
                choose_point.location.y = self.test_settings.vehicles[i][1]
                vehicle = self._world.try_spawn_actor(blueprint, choose_point)

                
                if vehicle:
                    self.init_vehicles.append((blueprint, choose_point, color, driver_id))
            ...
            for i in range(self.n_vehicles):
                ...
                while vehicle is None:
                    choose_point = np.random.choice(spawn_points)
                    vehicle = self._world.try_spawn_actor(blueprint, choose_point)
                self.init_vehicles.append((blueprint, choose_point, color, driver_id))
            ...
            if self.disable_two_wheels:
                blueprints = [x for x in blueprints if int(x.get_attribute("number_of_wheels")) == 4]
            spawn_points = self._map.get_spawn_points()
            spawn_points.pop(self.test_settings.start_pose)
            ...
            vehicle_info = self.fuzzer.get_vehicle_info()
            # print('vehicle1: ', len(vehicle_info))
            current_x = self.fuzzer.current_pose.location.x
            current_y = self.fuzzer.current_pose.location.y
            
            new_vehicle_info = self.fuzzer.vehicle_mutate(vehicle_info)
            # print('vehicle2: ', len(new_vehicle_info))

            legitimate_vehicle_info = []
            for i in range(len(new_vehicle_info)):
                temp_dis = np.linalg.norm([current_x - new_vehicle_info[i][1].location.x, current_y - new_vehicle_info[i][1].location.y])
                if temp_dis >= 1.90:
                    legitimate_vehicle_info.append(new_vehicle_info[i])
            # vehicle_info = copy.deepcopy(legitimate_vehicle_info)

            assert legitimate_vehicle_info
            del_indexes = []
            for i in range(len(legitimate_vehicle_info)):
                vehicle = None
                vehicle = self._world.try_spawn_actor(legitimate_vehicle_info[i][0], legitimate_vehicle_info[i][1])
            ...
            for del_ind in sorted(del_indexes, reverse=True):
                legitimate_vehicle_info.pop(del_ind)

            # print('vehicle number: ', len(legitimate_vehicle_info))
            self.fuzzer.current_vehicle_info = legitimate_vehicle_info
    ...
    def init(self, start=0, target=1, weather='random', n_vehicles=0, n_pedestrians=0):
        # while True:
        ...
        if self.replay:
            self._target_pose = self._map.get_spawn_points()[0]
            self._target_pose.location.x = self.test_settings.target_pose_x
            self._target_pose.location.y = self.test_settings.target_pose_y
            self.set_weather(PRESET_WEATHERS[self.test_settings.weather + 1])
        elif self.first_run:
            self._target_pose = self._map.get_spawn_points()[target]
            self.set_weather(weather)
        elif self.generative:
            self._target_pose = self._map.get_spawn_points()[self.test_settings.target_pose]
            self.set_weather(PRESET_WEATHERS[self.test_settings.weather + 1])
        else:
            self._target_pose = self._map.get_spawn_points()[self.fuzzer.current_envsetting[1]]
            self.set_weather(self.fuzzer.current_envsetting[2])
    ...
    def spawn_player(self):
        if self.replay:
            self._start_pose = self._map.get_spawn_points()[0]
            self._start_pose.location.x = self.test_settings.start_pose_x
            self._start_pose.location.y = self.test_settings.start_pose_y
            self._start_pose.rotation.yaw = self.test_settings.start_pose_yaw
        elif self.first_run:
            self._start_pose.location.x = self._start_pose.location.x + np.random.uniform(-0.15, 0.15)
            self._start_pose.location.y = self._start_pose.location.y + np.random.uniform(-0.15, 0.15)
            self._start_pose.rotation.yaw = self._start_pose.rotation.yaw + np.random.uniform(-5, 5)
        elif self.generative:
            spawn_points = self._map.get_spawn_points()
            self._start_pose = spawn_points[self.test_settings.start_pose]
            self._start_pose.location.x += self.test_settings.start_pose_x
            self._start_pose.location.y += self.test_settings.start_pose_y
            self._start_pose.rotation.yaw += self.test_settings.start_pose_yaw
        else:
            self._start_pose = self.fuzzer.get_pose()
            self._start_pose = self.fuzzer.mutation(self._start_pose)
            # print(self._start_pose)
        
        # # HACK:
        # self._start_pose.location.x += np.random.uniform(-0.2, 0.2)
        # self._start_pose.location.y += np.random.uniform(-0.2, 0.2)
        # self._start_pose.rotation.yaw += np.random.uniform(-3, 3)

        self._player = self._world.spawn_actor(self._vehicle_bp, self._start_pose)
        # Mutation here:
        # mutation_point = carla.Transform(Location(x=230, y=195, z=40), Rotation(yaw=180))
